...
# Constants
original_length = 0.025
cross_sectional_area = 19.625 * 1e-6  # mm^2 to m^2

# Load and clean the dataset
file_path = 'Failure_Al.csv'
data = pd.read_csv(file_path, skiprows=1, names=["Time (s)", "Displacement (mm)", "Force (kN)"])

# Convert columns to numeric
data["Displacement (mm)"] = pd.to_numeric(data["Displacement (mm)"], errors='coerce')
data["Force (kN)"] = pd.to_numeric(data["Force (kN)"], errors='coerce')

# Calculate Stress (Force / Area) and Strain (Displacement / Original Length)
data["Force (N)"] = data["Force (kN)"] * 1e3  # Convert kN to N
...
# Scientific Layout
plt.title("Stress-Strain Graph for Aluminum Sample", fontsize=16, fontweight='bold')
...
plt.ylim(0, ultimate_stress / 1e6 * 1.1)
